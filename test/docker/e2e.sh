...
if [ -n "${E2E_BREW:-}" ]; then
  dotfiles deps --only zellij 2>&1 | tail -5
  check '[ -x /home/linuxbrew/.linuxbrew/bin/brew ]' "Homebrew bootstrapped on Linux"
  check '[ -x /home/linuxbrew/.linuxbrew/bin/zellij ]' "zellij installed via Linuxbrew"
  out=$(dotfiles deps --dry-run --only zellij --only nvim)
  check 'echo "$out" | grep -Eq "^  ✓ zellij"' "zellij now detected"
